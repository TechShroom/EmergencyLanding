...
public abstract class EntityCollide extends ELEntity {
    ...
    public boolean testCollide(EntityCollide other) {
        ...
        double xGapBetweenObjs = Maths.projectLineAlongSurface(this.getRoll(), Math.pow(gapX*gapX+gapY*gapY,.5), gapX, false);
        double yGapBetweenObjs = Maths.projectLineAlongSurface(this.getRoll(), Math.pow(gapX*gapX+gapY*gapY,.5), gapY, false);
    ...
    /*public boolean testCollide(EntityCollide other) {
        ...
        double x_newGap = Maths.projectLineAlongSurface(this.getPitch(), angleToOther,
                Math.sqrt(gapX * gapX + gapY * gapY), false);
        double y_newGap = Maths.projectLineAlongSurface(this.getPitch(), angleToOther,
                Math.sqrt(gapX * gapX + gapY * gapY), true);

        double thisXLenOnNewGrid = this.getTex().getWidth();
        double thisYLenOnNewGrid = this.getTex().getHeight();

        double otherXLenOnNewGrid = Maths.projectLineAlongSurface(this.getPitch(),
                other.getPitch(), other.getTex().getWidth(), false)
                + Maths.projectLineAlongSurface(this.getPitch(), other.getPitch(),
                        other.getTex().getHeight(), true);
        double otherYLenOnNewGrid = Maths.projectLineAlongSurface(this.getPitch(),
                other.getPitch(), other.getTex().getWidth(), true)
                + Maths.projectLineAlongSurface(this.getPitch(), other.getPitch(),
                        other.getTex().getHeight(), false);

        boolean xTouch = x_newGap < (thisXLenOnNewGrid / 2 + otherXLenOnNewGrid/ 2);
        boolean yTouch = y_newGap < (thisYLenOnNewGrid / 2 + otherYLenOnNewGrid / 2);
        System.out.println();
